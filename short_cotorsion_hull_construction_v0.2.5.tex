...
with $d_n \in \Pi$ such that:
\begin{enumerate}
\item $d_n = \hat{g}_n - h_n$ for all $n \geq 1$
...
The condition $d_n = \hat{g}_n - h_n$ means that $d_n$ is congruent to an element of $G$ 
(modulo the natural embedding), but $d_n$ itself must be in $\Pi$ to satisfy the 
divisibility condition $d_n = (n+1) \cdot d_{n+1}$.
