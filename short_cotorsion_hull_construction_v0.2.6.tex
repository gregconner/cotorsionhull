...
$h_1 \in G$ such that for all $n \geq 1$:
\[
\hat{g}_1 - h_1 = n! \cdot (\hat{g}_n - h_n)
\]
where $h_n \in G$ is determined by this relation. This means the difference 
$\hat{g}_1 - h_1$ is divisible by $n!$ for all $n$, with quotient $\hat{g}_n - h_n$.
...
\item The remainders $(g_n)$ are compatible: $n \cdot g_m - m \cdot g_n \in G$ 
      for all $n, m \geq 1$.
\item Define $\hat{g}_n = g_{n!}$ for $n \geq 1$. There exists $h_1 \in G$ such that 
      for all $n \geq 1$:
      \[
      \hat{g}_1 - h_1 = n! \cdot (\hat{g}_n - h_n)
      \]
      where $h_n \in G$ is determined by this relation. This ensures the sequence 
      $(\hat{g}_n)$ represents a well-defined element in 
